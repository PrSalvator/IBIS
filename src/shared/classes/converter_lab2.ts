...
export class ConverterLab2 extends Converter {
    ...
    MakeSeed(block: string): string[] { //?
        ...
        const out = []
        out[0] = cesare.oneSideFunction(block, str1, 10);
        out[1] = cesare.oneSideFunction(block, str2, 10);
        out[2] = cesare.oneSideFunction(block, str3, 10);
    ...
    CheckSeed(block_in: string): string {
        ...
        const T = [];
        for (let i = 0; i < 4; i++) { // где-то тут мб в циклах неправильная длина стоит
            T[i] = block_in.substr(i * 4, 4);
        }

        for (let j = 0; j < 4; j++) { // где-то тут мб в циклах неправильная длина стоит
